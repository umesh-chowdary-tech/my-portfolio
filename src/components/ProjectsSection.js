...
const ProjectCard = ({ title, category, period, description, highlights, tech, links = [], note }) => (
  <article className="flex flex-col bg-white rounded-lg shadow-md border border-gray-100 p-5" data-test-id="projects-section-card">
    <div className="flex items-baseline justify-between gap-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">{category}</p>
      {period && <p className="text-xs text-gray-500 whitespace-nowrap">{period}</p>}
    </div>
    ...
    <div className="flex flex-wrap gap-2 mt-4">
      {tech.map((name) => (
        <span key={name} className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs font-medium">{name}</span>
      ))}
    </div>
    {(links.length > 0 || note) && (
      <div className="flex flex-wrap items-center gap-4 mt-auto pt-4">
        {links.map(({ label, href }) => (
          <a
            ...
            className="inline-flex items-center gap-1 text-sm font-semibold text-gray-900 hover:text-amber-700"
          >
  ...
  <div className="max-h-[75vh] md:max-h-[640px] overflow-y-auto pr-2 mt-12 md:mt-0" data-test-id="projects-section">
    ...
    {projectGroups.map((group) => (
      <section key={group.title} className="mt-6" data-test-id="projects-section-group">
        ...
        {group.note && <p className="text-sm text-gray-500">{group.note}</p>}
    ...
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-1 mt-6 text-sm font-semibold text-amber-700 hover:text-amber-900"
    >
